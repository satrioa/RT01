...
export async function createTransactionAction(
  ...
  if (parsed.data.category_id) {
    const { data: cat } = await supabase.from("categories").select("rt_id, type").eq("id", parsed.data.category_id).maybeSingle();
    ...
    const catType = (cat as { type: string }).type;
    if (catType !== "both" && catType !== parsed.data.type) return { ok: false, error: `Kategori ${catType} tidak cocok untuk ${parsed.data.type}` };
  }
