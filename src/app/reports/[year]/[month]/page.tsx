...
export default async function MonthlyReportDetailPage({
  ...
  const report = await getMonthlyReport(supabase, rtId, year, month).catch(() => null);
  if (!report) notFound();

  const { data: pockets } = await supabase.from("monthly_report_pockets").select("*").eq("monthly_report_id", report.id).order("pocket_name");
  ...
  // Fetch transactions for preview (limit 50)
  const { data: txs } = await supabase
    .from("transactions")
    .select("transaction_date, description, type, amount, pocket:pockets(name)")
    .eq("rt_id", rtId)
    ...
    <div className="min-h-dvh bg-background">
      <div className="mx-auto flex min-h-dvh w-full max-w-[430px] flex-col bg-background">
        <header className="flex items-center gap-3 border-b bg-card px-5 py-4">
          <Link href="/reports" className="flex size-9 items-center justify-center rounded-full border bg-card">
            <ArrowLeft className="size-4" />
          </Link>
          <div>
            <h1 className="text-sm font-semibold">Laporan Keuangan</h1>
            <p className="text-xs text-muted-foreground">{monthLabel} • {rtName} / RW {rwNumber}</p>
          </div>
        </header>
        ...
        <main className="flex flex-1 flex-col gap-4 p-5 pb-6">
          <Card className="overflow-hidden">
            <CardHeader className="pb-2">
              ...
              <p className="text-xs text-muted-foreground">RT {rtNumber} / RW {rwNumber} • Versi {report.version} • {report.status}</p>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-2">
                <Link href={`/api/reports/${year}/${String(month).padStart(2, "0")}/pdf`} className="flex-1">
                  <Button className="w-full rounded-xl">
                    <FileText className="size-4" /> Download PDF
                  </Button>
                </Link>
                <Link href={`/api/reports/${year}/${String(month).padStart(2, "0")}/excel`} className="flex-1">
              ...
              <div className="pt-2">
                <ReportActions year={year} month={month} status={report.status} />
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              {pocketRows.length === 0 ? (
                <p className="text-xs text-muted-foreground">Tidak ada kantong aktif.</p>
              ) : (
                pocketRows.map((p) => (
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm">Transaksi ({(txs as unknown[] | null)?.length ?? 0})</CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              {((txs as unknown as { transaction_date: string; description: string | null; type: string; amount: string; pocket: { name: string } | null }[] | null) ?? []).slice(0, 20).map((t, i) => (
                <div key={i} className="flex items-center justify-between rounded-xl border px-3 py-2 text-xs">
                  <div>
                    <p className="font-medium">{t.description ?? "-"}</p>
                    <p className="text-muted-foreground">{t.transaction_date} • {t.pocket?.name ?? "-"}</p>
                  </div>
              ...
              <p className="text-center text-[11px] text-muted-foreground">Pratinjau 20 transaksi • PDF berisi lengkap dengan saldo berjalan.</p>
            </CardContent>
          </Card>
